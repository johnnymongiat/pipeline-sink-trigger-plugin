...
public class BuildGraphPipelineSinkTrigger extends Trigger<BuildableItem> {
	...
	private DirectedGraph<AbstractProject<?,?>, String> toDirectedGraph(DependencyGraph dependencyGraph, AbstractProject<?,?> root, Set<String> exclusions) {
		final DirectedGraph<AbstractProject<?,?>, String> graph = 
				new DefaultDirectedGraph<AbstractProject<?,?>, String>(new EdgeFactory<AbstractProject<?,?>, String>() {
	...
	private List<AbstractProject<?,?>> toDepthFirstSequence(DependencyGraph dependencyGraph, AbstractProject<?,?> root, Set<String> exclusions) {
		...
		LOGGER.log(Level.INFO, String.format("The build pipeline graph rooted at '%s':%n%s", root.getName(), sb.toString()));
		return sequence;
	}
	
	private void triggerBuildOfSinkIfNecessary(List<AbstractProject<?,?>> sequence, AbstractProject<?,?> sink) {
		final List<String> listOfNonSuccessfulUpstreamProjectBuilds = new ArrayList<String>();
		long maxCompletionTimestamp = Long.MIN_VALUE;
		for (AbstractProject<?,?> project : sequence) {
			if (project.isBuilding() || project.isInQueue()) {
				LOGGER.log(Level.INFO, Messages.BuildGraphPipelineSinkTrigger_PipelineActive(sinkProjectName));
				return;
			}
